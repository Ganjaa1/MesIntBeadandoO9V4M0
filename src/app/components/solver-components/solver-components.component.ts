...
export class SolverComponentsComponent {
  ...
  searchRoutes() {
    ...
    this.vehicles.forEach((vehicle, index) => {
      vehicle.destinations = this.neighborhoodSearch(300000, vehicle.destinations);
      ...
      this.chartData.push({
        label: `Vehicle[${index}]`,
        data: [
          ...vehicle.destinations.map(cityNumber => ({ x: this.cities[cityNumber].x, y: this.cities[cityNumber].y }))
        ],
        borderColor: color,
        borderWidth: 1,
